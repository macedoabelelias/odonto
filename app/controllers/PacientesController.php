...
/* ADMIN / RECEPÇÃO / ACD */
if(in_array($nivel, ['administrador','admin','recepcionista','recepcao','acd'])){
    ...
    if(!empty($busca)){
        ...
        if($status == 'todos'){
            $pacientes = $this->pacienteModel->buscar($busca);
        } else {
            $pacientes = $this->pacienteModel->buscarAtivos($busca);
        }
    ...
    // 📋 SEM BUSCA
        ...
        if($status == 'todos'){
            $pacientes = $this->pacienteModel->listarTodosComStatus();
        } else {
...
public function excluir($id)
{
    ...
    if (!in_array($nivel, ['administrador','admin'])) {
        $_SESSION['erro'] = "Você não tem permissão para excluir pacientes.";
        header("Location: " . BASE_URL . "/pacientes");
        exit;
    }
...
{
    $this->pacienteModel->reativar($id);
