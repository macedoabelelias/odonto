...
<h5 class="modal-title">Procedimento no Dente</h5>
...
<select name="procedimento" class="form-control mb-2">
...
<script>
    ...
    console.log("Dente selecionado:", numero);

    let campoHidden = document.getElementById("denteSelecionado");
    if(campoHidden) campoHidden.value = numero;

    let campoVisual = document.getElementById("denteVisual");
    if(campoVisual) campoVisual.value = numero;

    // evita erro se função não existir
    if(typeof carregarDadosDoDente === "function"){
        carregarDadosDoDente(numero);
    }

    let modalEl = document.getElementById("modalProcedimento");
    if(modalEl){
